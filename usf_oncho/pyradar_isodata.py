...
#################################################
# This library was modified to support Python 3 #
#################################################
...
def quit_low_change_in_clusters(centers, last_centers, iter, theta_0):
    ...
    if centers.shape == last_centers.shape:
        ...
        if np.all(thresholds <= theta_0):  # percent of change in [0:1]
            quit = True
...
#        print "Isodata(info): Iteration:%s Num Clusters:%s" % (iter, k)
        ...
        centers, clusters_list = discard_clusters(img_class_flat,
                                                  centers, clusters_list, theta_m)
        centers, clusters_list = update_clusters(img_flat,
                                                 img_class_flat,
                                                 centers, clusters_list)
        max_k = centers.size

        if max_k <= (k / 2.0):  # too few clusters => split clusters
            centers, clusters_list = split_clusters(img_flat, img_class_flat,
                                                    centers, clusters_list,
                                                    theta_s, theta_m)

        elif max_k > (k * 2.0):  # too many clusters => merge clusters
        ...
        k = centers.size
###############################################################################
        if quit_low_change_in_clusters(centers, last_centers, iter, theta_0):
            break
